...
class KITTI360Bbox3D(KITTI360Object):
    ...
    def parseVertices(self, child):
        ...
        obj_bbox_center = (np.array(vertices[1]) + np.array(vertices[3]) + np.array(vertices[4]) +  np.array(vertices[6])) / 4
        obj_height = np.array([[0.0, 0.0, 1.0]])
        obj_width = np.array([[1.0, 0.0, 0.0]])
        obj_length = np.array([[0.0, 1.0, 0.0]])
        # print(self.bbox_center)
        # 1/0
        ...
        self.bbox_center = np.matmul(R, obj_bbox_center.transpose()).transpose() + T
        self.obj_height = np.linalg.norm(np.matmul(R, obj_height.transpose()).transpose())
        self.obj_width = np.linalg.norm(np.matmul(R, obj_width.transpose()).transpose())
        self.obj_length = np.linalg.norm(np.matmul(R, obj_length.transpose()).transpose())
        # print(np.matmul(R, obj_height.transpose()).transpose())
        # print(np.matmul(R, obj_width.transpose()).transpose())
        # print(np.matmul(R, obj_length.transpose()).transpose())
        # 1/0
        # print(self.bbox_center)
        # print(self.obj_height)
        # print(self.obj_width)
        # print(self.obj_length)
        # print(self.vertices)
        # 1/0
...
class Annotation3D:
    # Constructor
    def __init__(self, labelDir='', sequence='', posesDir = ''):

        # labelPath = glob.glob(os.path.join(labelDir, '*', '%s.xml' % sequence)) # train or test
        labelPath = os.path.join(labelDir, '%s.xml' % sequence)
        # if len(labelPath)!=1:
        ...
        self.frame_to_extrinsic = {}

        self.init_instance(labelPath)
        self.load_extrinsics(posesDir)


    def load_extrinsics(self, posesDir):
        poses_file = os.path.join(posesDir, self.sequence, "cam0_to_world.txt")

        if not os.path.isfile(poses_file):
            raise RuntimeError('%s does not exist!' % poses_file)
        
        with open(poses_file, 'r') as fid:
            lines = fid.readlines()
            for line in lines:
                # print(line.split())
                line = line.split()
                frame = int(line[0])
                cam2world = np.array([float(x) for x in line[1:]]).reshape(4, 4)
                world2cam = np.linalg.inv(cam2world)

                if frame not in self.frame_to_extrinsic:
                    self.frame_to_extrinsic[frame] = world2cam
                else:
                    # shouldn't happen
                    1/0

        print(f"Loaded camera extrinsics for {len(self.frame_to_extrinsic)} frames.")
